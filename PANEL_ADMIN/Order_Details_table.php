...
<!--                            <h4><a href="order_details_form.php">Add order details</a></h4>-->
                            ...
                            <table class="table" id="table2">
                                <thead>
                                 ...
                                 <?php
                                  $selectq = mysqli_query($connection, "select * from orders_details_tbl") or die(mysqli_error($connection));
                                $count=mysqli_num_rows($selectq);
                                echo $count    .   " Records Found";
                                while ($productrow = mysqli_fetch_array($selectq)) {
                                    echo "<tr>";
                                    echo "<td>{$productrow['orderdetails_id'] }</td>";
                                    echo "<td>{$productrow['order_id'] }</td>";
                                    echo "<td>{$productrow['tiffin_id'] }</td>";
                                    echo "<td>{$productrow['tiffin_name'] }</td>";
                                    echo "<td>{$productrow['quantity'] }</td>";
                                    echo "<td>{$productrow['rate'] }</td>";
