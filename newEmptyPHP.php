...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT user_id, user_name, user_email FROM users_tbl WHERE user_email = '$email' AND user_password = '$password'";
    ...
    if(mysqli_num_rows($result) > 0)
    {
       while($row = mysqli_fetch_assoc($result))
       {
         ...
         if(isset($_POST['rememberme']))
         {
            ...
            if($row['type_id']==1)
            {
             header('location:localhost/htdocs/TIFFINSELLER_P/Welcome.php');
            }
         else
            {
